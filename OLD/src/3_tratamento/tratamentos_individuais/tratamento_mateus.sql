...
SELECT 
    ...
    CASE 
        WHEN passenger_count IS NULL THEN 1              -- Imputação padrão para falha de telemetria
        WHEN passenger_count = 0 THEN 0                  -- Preserva o zero identificado pelo André (possível cancelamento)
        WHEN passenger_count > 9 THEN 1                  -- Correção para erros absurdos acima do limite de vans (9)
        ELSE passenger_count 
    END AS passenger_count_clean,
    
    trip_distance,
    COALESCE(RatecodeID, 99) AS RatecodeID_clean,        -- Padronização do dicionário para nulos
